...
void DetectarLabels(void)
{
    ...
    while (Look != EOF)
    {
        ...
        else
        {
            ...
            /* Instruções de 3 argumentos e 2 linhas: instr (), (), () -> [...] + [...] */
			...
			free(str_tmp2);
                ...
                */
                case ADDN_CODE:
                    ...
                    //Registrador onde será escrito
                    str_tmp1 = parser_GetItem_s();
                    val1 = BuscaRegistrador(str_tmp1);
                    free(str_tmp1);

                    //Registrador com o primeiro dado
                    parser_Match(',');
                    str_tmp2 = parser_GetItem_s();
                    val2 = BuscaRegistrador(str_tmp2);
                    free(str_tmp2);
                    
                    //Numero a ser Escrito
                    parser_Match(',');
                    val3 = RecebeNumero();
                    ...
                    sprintf(str_msg,"%s%s%s0000",ADDN,str_tmp1,str_tmp2);
                    parser_Write_Inst(str_msg,end_cnt);
                    end_cnt += 1;
...
unsigned short RecebeNumero(void)
{
    ...
    if (Look == '#')
    {
        ...
        else if (parser_IsDigitOrSignal(Look))  /* Numero com complemento de 2 : #+1234 ou #-1234 */
        {
            str_tmp = parser_GetPosNeg_s();
            ret = atoi(str_tmp);
            free(str_tmp);
            return ret;
