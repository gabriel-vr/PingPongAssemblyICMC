...
int parser_IsDigitOrSignal(char c)
{
...
/* Pega Número com ou sem sinal*/
char * parser_GetPosNeg_s(void)
{
    char *Token;
    Token = (char *)calloc(STRTAM,sizeof(char));
    int i=0;
    if (!parser_IsDigitOrSignal(Look)) parser_Expected("Número com ou sem sinal");
    while (parser_IsDigitOrSignal(Look))
    {
        Token[i]=Look;
        parser_GetChar();
        i++;
